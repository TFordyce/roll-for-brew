-- Effect Invocation -- Group A: Saucerer's Apprentice, Genie in the Teapot,
-- Brew-merang (issue #316, child of spec #302 / ADR 0005 §10).
--
-- A follow-up slice of the effect-application rebuild, landing on the shared
-- integration branch rebuild/effect-resolver after #307 (migration 0080,
-- Cast-Log resolution phase) and #308 (migration 0081, counterspell-unwind +
-- recursive negate fixpoint), whose machinery this slice extends.
--
-- Representation (spec §10): the invoking card's spell_casts row carries a
-- pointer in cast_inputs -- copied_cast_id (Saucerer), invoked_card (Genie),
-- seized_cast_id (Brew-merang). No new effect_kind, no new table. resolve_round
-- DERIVES the extra effect from the pointer, the same way it derives negation.
-- Invocation is whole-cast: every effect row of the source, or none.
--
--   * Saucerer's Apprentice (Reaction, CARD): cast_reaction_spell_card gains a
--     name-based branch that records { copied_cast_id, copy_inputs } on a
--     single effect_kind = NULL row. All fresh copy RNG (a copied 1d6 / d20
--     re-rolls) is drawn AT CAST TIME into cast_inputs.copy_inputs so the
--     resolver stays pure. resolve_round Phase 0 materialises one concrete
--     spell_casts row per source effect row -- caster = the Apprentice, target
--     = the Apprentice caster (a card-targeted copy, e.g. a counter, keeps the
--     source's parent_cast_id and re-resolves against the same card), seq after
--     the source -- then every downstream phase carries them for free. Copying
--     a negated / broken-chain source is a no-op step.
--
--   * Genie in the Teapot (Action, CARD, Epic): cast_spell_card gains a
--     p_invoked_card_name arg (=> signature change, DROP + CREATE 5-arg).
--     Dedicated validation branch: the named card must be a non-Epic Action
--     card, not Genie itself, with effect rows, whose sole edition instance is
--     in_deck (RFB50 otherwise). The instance is NOT moved (ethereal -- two
--     Genies of one card both resolve). The Genie's own spell_casts rows carry
--     the named card's effect_kind / effect_params + cast_inputs.invoked_card.
--
--   * Brew-merang (Reaction, CARD): cast_reaction_spell_card name-based branch
--     records { seized_cast_id } on an effect_kind = NULL row targeting another
--     player's cast. resolve_round Phase 0 retargets the seized cast group to
--     its own caster (multi-target casts collapse -- N-1 rows negated, one
--     retargeted CASTER) and writes seized_by_cast_id. seize is a
--     resolver-derived outcome inside the #308 recursive negate fixpoint: a
--     countered Brew-merang never seizes; Brew-merangs seize Brew-merangs.
--
--   * Wards: block_copy (Bag for Life) blocks Saucerer's Apprentice /
--     Brew-merang from invoking a cast the holder made -- the card is still
--     burned, outcome `blocked` + a `warded` Trace step. Genie is out of reach
--     by construction (it names a card, not a cast).
--
--   * No meta-invocation: Saucerer's Apprentice / Brew-merang cannot target a
--     cast made by Saucerer's Apprentice / Genie / Brew-merang (RFB49).
--
-- All three cards are un-benched (spell_deck_instances.location 'benched' ->
-- 'in_deck'). New error codes: RFB49 (invalid invocation target -- a
-- meta-invocation, or a Brew-merang aimed at the caller's own cast), RFB50
-- (ineligible Genie card name).
--
-- Migration numbering: master's highest is 0077; rebuild/effect-resolver runs
-- 0078-0092. This is 0093. Re-check the number at the #303 integrate step.
--
-- Documented pragmatic corners (exotic interactions, flagged for the
-- integrator -- see the PR body):
--   - A copied eager roll-input cast (advantage / disadvantage / forced_reroll
--     / roll_flip) has its before->after synthesised at cast time onto the
--     Apprentice caster from their current layer-0 roll + fresh RNG. roll_swap
--     copies as a zero-impact step (no coherent "swap on one player").
--   - Brew-merang seizing an already-executed eager roll-input cast unwinds it
--     on the original target (resolver adopts the recorded `before`) and does
--     not re-apply it on the caster.
--   - Brew-merang-seizes-Brew-merang chains one level deep in the resolver.

-- ---------------------------------------------------------------------------
-- 1. Un-bench the three cards (migration 0074 parked them at 'benched').
--    Guarded on location so this is a no-op if 0074 never ran.
-- ---------------------------------------------------------------------------
update public.spell_deck_instances sdi
 ...
 where sc.id = sdi.card_id
   and sc.name in ('Saucerer''s Apprentice', 'Genie in the Teapot', 'Brew-merang')
   and sdi.location = 'benched';

-- ---------------------------------------------------------------------------
-- 2. _rr_invocation_resolution(uuid) -- one row per Saucerer's Apprentice /
--    Brew-merang invoking cast in the round, with its derived resolution.
--
--    An invoking cast is a spell_casts row with effect_kind IS NULL carrying
--    cast_inputs.copied_cast_id (Saucerer) or cast_inputs.seized_cast_id
--    (Brew-merang) and parent_cast_id = the specific source spell_casts row.
--
--    Negation reuses the #308 fixpoint: an invoking cast is negated iff a
--    higher-seq, succeeded, non-negated contested_negate targets ITS OWN cast
--    group. `source_broken` is true when the source cast group is itself
--    negated by a live counter or is another (negated) invoking cast.
-- ---------------------------------------------------------------------------
drop function if exists public._rr_invocation_resolution(uuid);
create function public._rr_invocation_resolution(p_round_id uuid)
returns table (
  invocation_cast_id uuid,
  invocation_kind text,          -- 'copy' | 'seize'
  invocation_seq bigint,
  invocation_caster text,
  invocation_negated boolean,
  source_parent_cast_id uuid,    -- the exact targeted spell_casts row
  source_group uuid,             -- card_instance_id of the copied / seized cast
  source_caster text,            -- caster of the source cast (seize retarget)
  source_broken boolean,         -- source negated / itself a dead invocation
  ward_cast_id uuid,             -- block_copy ward held by source_caster
  ward_card_name text
)
language plpgsql
stable
...
declare
  v_room_id uuid;
  v_set jsonb := '[]'::jsonb;    -- unified working set: counters + invocations
  v_n integer;
  v_idx integer;
  v_pass integer;
  v_changed boolean;
...
begin
  ...
  -- Gather every contested_negate cast (for the negation fixpoint) plus every
  -- invoking cast, into one seq-ordered jsonb working set.
  select coalesce(jsonb_agg(entry order by (entry->>'seq')::bigint), '[]'::jsonb)
    into v_set
    from (
      -- contested_negate casts -- only what the fixpoint needs
      select jsonb_build_object(
       ...
       where c.round_id = p_round_id
         and c.effect_kind = 'contested_negate'
      union all
      -- invoking casts
      select jsonb_build_object(
               'row_kind', 'invocation',
               ...
               'seq', c.seq,
               'own_group', c.card_instance_id,
               'kind', case when c.cast_inputs ? 'seized_cast_id' then 'seize' else 'copy' end,
               'caster', c.caster_id,
               'source_parent_cast_id', c.parent_cast_id,
               'source_group', src.card_instance_id,
               'source_caster', src.caster_id,
               'is_negated', false
             ) as entry
        from public.spell_casts c
        left join public.spell_casts src on src.id = c.parent_cast_id
       where c.round_id = p_round_id
         and c.effect_kind is null
         and (c.cast_inputs ? 'copied_cast_id' or c.cast_inputs ? 'seized_cast_id')
    ) s;

  v_n := jsonb_array_length(v_set);
  if v_n = 0 then
    return;
  end if;

  -- Fixpoint: an entry is negated iff some higher-seq contested_negate targets
  -- ITS OWN cast group, succeeded, and is not itself negated. Bounded at 2n+2
  -- passes, converges because every counter targets a strictly-lower seq.
  --
  -- This is deliberately the same #308 negation rule as
  -- _rr_cast_log_resolution (migration 0081) -- the two implement one rule
  -- over the shared { seq, own_group, target_group, succeeded, is_negated }
  -- shape and MUST evolve together. Kept as a local copy rather than a shared
  -- helper only to avoid re-emitting 0081's function in this slice; extracting
  -- `_rr_negate_fixpoint(jsonb) -> jsonb` is a clean follow-up.
  for v_pass in 1 .. (2 * v_n + 2) loop
    ...
    for v_idx in 0 .. v_n - 1 loop
      ...
      v_neg := exists (
        select 1
          from jsonb_array_elements(v_set) d
         where d.value->>'row_kind' = 'counter'
      ...
      if v_neg is distinct from (v_c->>'is_negated')::boolean then
        v_set := jsonb_set(v_set, array[v_idx::text, 'is_negated'], to_jsonb(v_neg));
        v_changed := true;
      end if;
    end loop;
  ...
  -- Groups killed by a live counter.
  select coalesce(array_agg(distinct (d.value->>'target_group')::uuid), array[]::uuid[])
    into v_negated_groups
    from jsonb_array_elements(v_set) d
   where d.value->>'row_kind' = 'counter'
     and (d.value->>'succeeded')::boolean
     and not (d.value->>'is_negated')::boolean;

  -- ...plus any invoking cast that is itself negated (a dead Brew-merang whose
  -- seize another Brew-merang pointed at).
  select coalesce(v_negated_groups || array_agg(distinct (d.value->>'own_group')::uuid), v_negated_groups)
    into v_negated_groups
    from jsonb_array_elements(v_set) d
   where d.value->>'row_kind' = 'invocation'
     and (d.value->>'is_negated')::boolean;

  for v_idx in 0 .. v_n - 1 loop
    v_c := v_set -> v_idx;
    if v_c->>'row_kind' <> 'invocation' then
      continue;
    end if;

    invocation_cast_id    := (v_c->>'cast_id')::uuid;
    invocation_kind       := v_c->>'kind';
    invocation_seq        := (v_c->>'seq')::bigint;
    invocation_caster     := v_c->>'caster';
    invocation_negated    := (v_c->>'is_negated')::boolean;
    source_parent_cast_id := (v_c->>'source_parent_cast_id')::uuid;
    source_group          := (v_c->>'source_group')::uuid;
    source_caster         := v_c->>'source_caster';
    source_broken         := (v_c->>'source_group') is null
                             or (v_c->>'source_group')::uuid = any (v_negated_groups);

    ward_cast_id   := null;
    ward_card_name := null;
    if source_caster is not null then
      select sae.source_cast_id, scw.name
        into ward_cast_id, ward_card_name
        from public.spell_active_effects sae
        join public.spell_cards scw on scw.id = sae.card_id
       where sae.room_id = v_room_id
         and sae.target_player_id = source_caster
...
revoke execute on function public._rr_invocation_resolution(uuid) from public, anon;
grant execute on function public._rr_invocation_resolution(uuid) to authenticated;

comment on function public._rr_invocation_resolution(uuid) is
  'Issue #316: one row per Saucerer''s Apprentice / Brew-merang invoking cast '
  '(effect_kind NULL + cast_inputs.copied_cast_id / seized_cast_id) with its '
  'derived resolution -- whether the invoking cast is itself negated (recursive, '
  'via the #308 fixpoint), the source cast group and its caster, whether the '
  'source is broken (negated / a dead invocation), and any block_copy ward held '
  'by the source caster. resolve_round Phase 0 consumes this.';

-- ---------------------------------------------------------------------------
-- 3. _rr_build_copy_inputs(round, source_cast_id, apprentice) -- draws ALL of
--    a Saucerer's Apprentice copy's fresh server RNG at cast time and returns
--    the cast_inputs.copy_inputs bundle. resolve_round only ever MATERIALISES
--    rows from these recorded values, so it stays pure and idempotent.
--
--      * a copied contested_negate re-rolls its d20 (dc carried from the
--        original, which already checked the same target card's tier)
--      * a copied dice_modifier re-rolls its dice into `dice_roll`
--      * a copied eager roll-input cast (advantage / disadvantage /
--        forced_reroll / roll_flip) gets a synthesised before->after
--        `roll_transform` landing on the Apprentice caster, from their current
--        layer-0 roll + fresh RNG. roll_swap copies as a zero-impact transform
--        (there is no coherent "swap on a single player").
-- ---------------------------------------------------------------------------
drop function if exists public._rr_build_copy_inputs(uuid, uuid, text);
create function public._rr_build_copy_inputs(
  p_round_id uuid, p_source_cast_id uuid, p_apprentice text
)
returns jsonb
language plpgsql
volatile
...
declare
  v_group uuid;
  v_by_cast jsonb := '{}'::jsonb;   -- { source spell_casts.id::text : { row inputs } }
  v_row record;
  v_row_out jsonb;
  v_appr_roll integer;
  ...
  v_order integer;
begin
  select card_instance_id into v_group
    from public.spell_casts where id = p_source_cast_id;
  if v_group is null then
    return jsonb_build_object('by_cast', v_by_cast);
  end if;

  select value into v_appr_roll
    from public.rolls
   where round_id = p_round_id and player_id = p_apprentice and layer = 0;

  -- One entry per source effect row, keyed by its spell_casts.id, so a
  -- multi-effect / fan-out source never has two rows' RNG collide (0079 /
  -- 0081 key recorded RNG the same way). resolve_round Phase 0a looks each
  -- copy row's inputs up by the id it copied.
  for v_row in
    select sc.id, sc.effect_kind, sc.effect_params, sc.cast_inputs
      from public.spell_casts sc
     where sc.card_instance_id = v_group
     order by sc.seq
  loop
    v_row_out := null;

    if v_row.effect_kind = 'contested_negate' then
      v_row_out := jsonb_build_object(
        'dc_d20', floor(random() * 20 + 1)::integer,
        'dc', coalesce((v_row.cast_inputs->>'dc')::integer, 10));

    elsif v_row.effect_kind = 'dice_modifier' then
      v_dice_count := coalesce((regexp_match(v_row.effect_params ->> 'dice', '^(\d+)d(\d+)$'))[1]::integer, 1);
      v_dice_sides := coalesce((regexp_match(v_row.effect_params ->> 'dice', '^(\d+)d(\d+)$'))[2]::integer, 6);
      v_total := 0;
      for i in 1 .. v_dice_count loop
        v_total := v_total + floor(random() * v_dice_sides + 1)::integer;
      end loop;
      v_row_out := jsonb_build_object('dice_roll', v_total);

    elsif v_row.effect_kind in ('advantage', 'disadvantage', 'forced_reroll', 'roll_flip', 'roll_swap')
          and v_appr_roll is not null then
      v_order := case v_row.effect_kind
        when 'advantage' then 1 when 'disadvantage' then 1
        when 'forced_reroll' then 2 when 'roll_flip' then 3 else 4 end;

      if v_row.effect_kind in ('advantage', 'disadvantage') then
        v_x := floor(random() * 20 + 1)::integer;
        v_after := case when v_row.effect_kind = 'advantage'
          then greatest(v_appr_roll, v_x) else least(v_appr_roll, v_x) end;
        v_row_out := jsonb_build_object('roll_transform', jsonb_build_object(
          'kind', v_row.effect_kind, 'order', v_order,
          'dice', jsonb_build_array(v_appr_roll, v_x),
          'players', jsonb_build_array(jsonb_build_object(
            'player_id', p_apprentice, 'before', v_appr_roll, 'after', v_after))));
      elsif v_row.effect_kind = 'forced_reroll' then
        v_x := floor(random() * 20 + 1)::integer;
        v_row_out := jsonb_build_object('roll_transform', jsonb_build_object(
          'kind', 'forced_reroll', 'order', v_order,
          'players', jsonb_build_array(jsonb_build_object(
            'player_id', p_apprentice, 'before', v_appr_roll, 'after', v_x))));
      elsif v_row.effect_kind = 'roll_flip' then
        v_row_out := jsonb_build_object('roll_transform', jsonb_build_object(
          'kind', 'roll_flip', 'order', v_order,
          'players', jsonb_build_array(jsonb_build_object(
            'player_id', p_apprentice, 'before', v_appr_roll, 'after', 21 - v_appr_roll))));
      else
        -- roll_swap: zero-impact copy (there is no coherent "swap on one
        -- player"; documented in the migration header).
        v_row_out := jsonb_build_object('roll_transform', jsonb_build_object(
          'kind', 'roll_swap', 'order', v_order,
          'players', jsonb_build_array(jsonb_build_object(
            'player_id', p_apprentice, 'before', v_appr_roll, 'after', v_appr_roll))));
      end if;
    end if;

    if v_row_out is not null then
      v_by_cast := v_by_cast || jsonb_build_object(v_row.id::text, v_row_out);
    end if;
  end loop;
...
revoke execute on function public._rr_build_copy_inputs(uuid, uuid, text) from public, anon;
grant execute on function public._rr_build_copy_inputs(uuid, uuid, text) to authenticated;

comment on function public._rr_build_copy_inputs(uuid, uuid, text) is
  'Issue #316: draws every fresh server-RNG a Saucerer''s Apprentice copy needs '
  '(re-rolled d20 / dice, and a synthesised roll_transform for a copied eager '
  'roll-input cast landing on the Apprentice caster) at cast time, so '
  'resolve_round only materialises rows from recorded values and stays pure.';

-- ---------------------------------------------------------------------------
-- 4. cast_reaction_spell_card -- re-emitted from 0088 with a name-based branch
--    for Saucerer's Apprentice / Brew-merang ahead of the Tea-tally Spent block.
-- ---------------------------------------------------------------------------
...
comment on function public.cast_reaction_spell_card(uuid, text, uuid, integer) is
  'Reaction-window cast (issues up to #316). Name-based branches: Tea-tally '
  'Spent (p_spend_amount, RFB44/RFB45) and the Effect Invocation Reactions '
  'Saucerer''s Apprentice / Brew-merang (#316) -- a single effect_kind = NULL '
  'invoking row carrying cast_inputs.copied_cast_id / seized_cast_id; RFB49 on '
  'meta-invocation or a Brew-merang aimed at the caller''s own cast. '
  'p_spend_amount is Tea-tally '
...
-- ---------------------------------------------------------------------------
-- 5. cast_spell_card -- re-emitted from 0089, signature grows p_invoked_card_name
--    (Genie in the Teapot); dedicated Genie validation branch before the
--    target-stamp switch, rows tagged cast_inputs.invoked_card after the loop.
-- ---------------------------------------------------------------------------
-- issue #316: signature grows p_invoked_card_name (Genie in the Teapot) --
-- DROP the 4-arg form so the overload set stays a single function.
drop function if exists public.cast_spell_card(uuid, text, text[], integer);
...
comment on function public.cast_spell_card(uuid, text, text[], integer, text) is
  'Pre-roll Action cast (issues up to #316). p_invoked_card_name is Genie in '
  'the Teapot only: the non-Epic Action card it names and resolves as if '
  'played (the named instance is never moved -- ethereal); RFB50 on an '
  'ineligible name (Epic / non-Action / Genie itself / no invokable effect / '
  'sole instance not in_deck / target not chosen now).';

-- ---------------------------------------------------------------------------
-- 6. resolve_round(uuid) -- re-emitted from 0092 with Phase 0a (materialise
--    Saucerer's Apprentice copies, before Phase 1) and Phase 0b (Brew-merang
--    seize retarget + copy / seize / block_copy outcome + Trace, after the
--    #344 Pre-pass). Issue #316.
-- ---------------------------------------------------------------------------
