-- Issue #285: Kettle Crash's reset_persistent_modifier effect is inert.
--
-- Kettle Crash (Epic, TABLE, Action — "All players' modifiers reset to 0")
-- has a spell_card_effects row (reset_persistent_modifier / TABLE, migration
-- 0033:113), but cast_spell_card's "single table-wide event, no per-player
-- row" branch just inserts a bare spell_casts row for that kind and nothing
-- ever reads it. Casting Kettle Crash resets nothing.
--
-- Fix: special-case effect_kind = 'reset_persistent_modifier' in that branch
-- to run the same table-wide modifier reset Wild Brew Surge branch 1 already
-- does (update public.room_players set modifier = 0 where room_id = ...) —
-- immediately, at cast time; it's persistent, not round-scoped, so no
-- close_round fan-out is needed — then still write the audit spell_casts row.
--
-- The card text's "the day starts again, mechanically" half stays
-- deliberately unimplemented (documented simplification since 0033).
--
-- cast_spell_card latest prior definition: 0069. Redefined verbatim below
-- with only the ~5-line reset_persistent_modifier special-case added.
--
-- Numbered 0076: 0074 is claimed by PR #322 (issue #284, bench the 39
-- non-working cards) and 0075 by the concurrent Yorkshire Terror fix
-- (issue #286). #322's 0074 adds the 'benched' location and parks Kettle
-- Crash; the un-bench UPDATE at the foot of this file flips it back once
-- both are live (guarded, so a no-op if #322 hasn't landed).
  ...
  loop
    ...
    if v_effect.target_role = 'CASTER' then
      ...
      if v_effect.effect_kind = 'reset_persistent_modifier' then
        -- Kettle Crash (#285): persistent, not round-scoped, so apply the
        -- reset live at cast time — no close_round fan-out. The spell_casts
        -- insert just below is the audit trail. (The card text's "the day
        -- starts again, mechanically" half stays unimplemented, per 0033.)
        update public.room_players set modifier = 0 where room_id = v_room_id;
      end if;
...
-- The bench migration (#284) parks Kettle Crash's deck instance at
-- location='benched' so draw_spell_card skips it. Now that the card works,
-- flip it back into the draw pool. Guarded on location so this is a no-op
-- if #284 hasn't landed yet, and skips an instance a player currently holds.
update public.spell_deck_instances sdi
   set location = 'in_deck', held_by_player = null
  from public.spell_cards sc
 where sc.id = sdi.card_id
   and sc.name = 'Kettle Crash'
