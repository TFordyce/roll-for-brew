-- Tier A primitive 3 -- Conditional advantage (issue #319, child of the
-- effect-application rebuild spec #302 / ADR 0005).
--
-- Gambler's Infusion (v2, Epic, SELF, Action): "Roll your d20. On a 15+, roll
-- a second d20 and take the higher. On a 5 or lower, roll a second d20 and
-- take the lower." -- i.e. plain advantage / disadvantage, but which one (or
-- neither) is chosen by the *first* die rather than fixed by the card.
--
-- Built entirely on the #306 eager shim. The card gets one spell_card_effects
-- row -- effect_kind 'advantage' (no new kind), carrying a `condition` object
-- in effect_params:
--
--   effect_params -> 'condition' = {
--     "advantage_at_or_above":    15,   -- first die >= this  -> advantage
--     "disadvantage_at_or_below":  5    -- first die <= this  -> disadvantage
--   }                                   -- between the two    -> neither
--
-- submit_roll / submit_roll_as evaluate that condition against the first d20
-- and fold the chosen branch into the existing advantage/disadvantage
-- resolution -- so both-cancel (a plain opposing cast on the same player) and
-- every other rule of the base shim are inherited unchanged. The branch taken
-- is recorded into the cast's cast_inputs.roll_transform:
--
--   roll_transform = {
--     "kind": "advantage", "order": 1, "cancelled": <bool>,
--     "condition": {
--       "first_die": <d20>,
--       "branch": "advantage" | "disadvantage" | "none",
--       "advantage_at_or_above": 15, "disadvantage_at_or_below": 5
--     },
--     "dice": [<d20>, <d20>?],
--     "players": [{ "player_id", "before": <first die>, "after": <kept> }]
--   }
--
-- resolve_round Phase 3 already adopts `players[].after` with no computation
-- (issue #306); this migration only teaches its Trace step to name the branch
-- that was taken -- `advantage` / `disadvantage` for a met threshold, and a
-- zero-impact `conditional_advantage` step when neither was met.
--
-- Roll-domain ward: the base advantage shim has no ward interaction (a ward
-- guard on submit_roll is deferred, #335), so conditional advantage inherits
-- none -- nothing to add here.
--
-- Un-benches Gambler's Infusion (#284 parks it at location = 'benched').

-- ---------------------------------------------------------------------------
-- 1. Effect row for Gambler's Infusion.
-- ---------------------------------------------------------------------------
insert into public.spell_card_effects (card_id, target_role, effect_kind, effect_params)
select sc.id, 'CASTER', 'advantage',
       jsonb_build_object('condition', jsonb_build_object(
         'advantage_at_or_above', 15,
         'disadvantage_at_or_below', 5
       ))
  from public.spell_cards sc
 where sc.name = 'Gambler''s Infusion'
   and not exists (
     select 1 from public.spell_card_effects e where e.card_id = sc.id
   );

-- ---------------------------------------------------------------------------
-- 2. submit_roll(uuid) -- re-emitted from 0079 with conditional-advantage
--    evaluation folded into the advantage / disadvantage block.
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_eff_advantage <> v_eff_disadvantage then
  ...
  if v_eff_advantage or v_eff_disadvantage or v_condition is not null then
...
-- ---------------------------------------------------------------------------
-- 3. submit_roll_as(uuid, text) -- admin-puppet twin, kept in lockstep.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. resolve_round(uuid) -- re-emitted from 0093. Only change: Phase 3's
--    roll-input Trace step names the conditional-advantage branch that was
--    taken (advantage / disadvantage / a zero-impact conditional_advantage
--    step when the first die met neither threshold), and carries the recorded
--    `condition` object as a step extra. Roll adoption is unchanged.
-- ---------------------------------------------------------------------------
  ...
  for v_i in 1 .. coalesce(array_length(v_players, 1), 0) loop
    ...
    loop
      ...
      v_trace := v_trace || jsonb_build_array(public._rr_trace_step(
        ...
        case when v_row.condition is not null
