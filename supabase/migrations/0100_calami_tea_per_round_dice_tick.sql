-- Calami-Tea: per-round dice tick (issue #289, spec #302 Tier B primitive 6 /
-- ADR 0005 -- "per-round dice tick: persistent effect rolls a fresh dN each
-- round vs the roll; per-tick RNG recorded in the Cast Log").
--
-- Card (Rare, OPPONENT / CHOSEN_PLAYERS, Action): "Choose up to 3 players. For
-- the next 3 rounds, they each subtract 1d4 from their rolls."
--
-- Before this migration Calami-Tea was a stand-in: a single `flat_modifier`
-- {delta: -2, max_targets: 3} effect row (migration 0033), duration 3 -- a
-- flat -2 every round, no per-round dice. This replaces it with a real
-- per-round tick built on the finished resolver pipeline (#303-#321).
--
-- Model -- the Bitter Leech (#342) anchor + Phase-pre synthesis pattern, in
-- the roll domain instead of the modifier domain:
--
--   * One new effect_kind, `per_round_dice_tick`, effect_params
--     {die, sign, max_targets}. Calami-Tea's spell_card_effects row becomes
--     CHOSEN_PLAYERS / per_round_dice_tick {die: 4, sign: -1, max_targets: 3};
--     duration_rounds stays 3, polarity stays negative.
--
--   * Casting it goes through the generic CHOSEN_PLAYERS branch of
--     cast_spell_card unchanged: one anchor `per_round_dice_tick` cast per
--     chosen player + one spell_active_effects projection row per target
--     (rounds_remaining => 3 from the card's duration_rounds via
--     record_active_effect_if_persistent). The anchor carries no
--     roll_transform, so it is inert past the projection. No cast_spell_card
--     special-case, and the existing max_targets guard still enforces "up to
--     3" off effect_params.max_targets.
--
--   * resolve_round gains a Phase 3-pre: for each live per_round_dice_tick
--     active effect, once per (round, source cast, generation), roll a fresh
--     1dN and write a child cast carrying it in cast_inputs.roll_transform
--     (kind per_round_dice_tick, order 2). The Phase 3 walk subtracts that
--     recorded die from the target's roll (floor 1) and emits a `dice_tick`
--     Trace step. Deterministic on re-resolve -- the recorded die is reused,
--     not re-rolled -- so the per-tick RNG lives in the Cast Log (spec Tier B
--     primitive 6). Liveness (cast round + next 2, then stop) is
...
--     `warded` step is emitted, mirroring Phase 4b-pre's Bitter Leech ward
--     pre-pass. Re-evaluated every round off the live ward map, so a later
--     tick after the ward expires still applies.
--
--   * The active-effect projection row also drives the effect-duration badge
--     for free (get_room_active_effects reads _rr_active_effects_as_of) -- the
--     target sees a 3-round countdown dot, the "For the next 3 rounds" signal.
--
-- Client: roundRecap.ts gains the `dice_tick` sentence template.
--
-- Migration numbering: master's highest is 0099 (0099_targeting_skip, #321).
-- This is 0100. No open PR claims 0100 at time of writing; re-verify at the
-- #303 integrate gate before any release-branch merge.

-- ---------------------------------------------------------------------------
-- 1. Un-bench Calami-Tea. Guarded on location so this is a no-op where 0074
...
update public.spell_deck_instances sdi
 ...
 where sc.id = sdi.card_id
   and sc.name = 'Calami-Tea'
   and sdi.location = 'benched';

-- ---------------------------------------------------------------------------
-- 2. effect_kind CHECK constraints -- add `per_round_dice_tick` to all three.
--    Must run BEFORE the effect-row insert in section 3. Lists carried
--    forward from 0099 (spell_card_effects / spell_casts / spell_active_
--    effects).
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Calami-Tea effect row: replace the flat_modifier stand-in (0033) with
--    one CHOSEN_PLAYERS per_round_dice_tick row. duration_rounds / polarity
--    re-asserted (0033 already set them) so this migration is self-contained.
-- ---------------------------------------------------------------------------
delete from public.spell_card_effects
 where card_id in (select id from public.spell_cards where name = 'Calami-Tea');

insert into public.spell_card_effects (card_id, target_role, effect_kind, effect_params, ordinal)
select id, 'CHOSEN_PLAYERS', 'per_round_dice_tick',
       jsonb_build_object('die', 4, 'sign', -1, 'max_targets', 3), 0
  from public.spell_cards where name = 'Calami-Tea';

update public.spell_cards
   set duration_rounds = 3, polarity = 'negative'
...
begin
  ...
  loop
    ...
    v_dt_roll := floor(random() * v_dt.die + 1)::integer;
