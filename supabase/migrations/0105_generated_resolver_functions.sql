...
--   _rr_active_ward_gate
--   _rr_apply_fixed_roll
...
-- BEGIN db/sql/functions/_rr_active_ward_gate.sql
...
-- END db/sql/functions/_rr_active_ward_gate.sql

-- BEGIN db/sql/functions/_rr_apply_fixed_roll.sql
...
-- END db/sql/functions/_rr_apply_fixed_roll.sql

-- BEGIN db/sql/functions/_rr_cast_log_resolution.sql
...
-- END db/sql/functions/_rr_cast_log_resolution.sql

-- BEGIN db/sql/functions/_rr_pick_lowest.sql
...
-- END db/sql/functions/_rr_pick_lowest.sql
...
-- BEGIN db/sql/functions/_rr_trace_step.sql
...
-- END db/sql/functions/_rr_trace_step.sql
...
--
-- Authoritative round resolution: the ordered Resolver pipeline (Phases
-- 0a/0b Effect Invocation, 1 Cast-Log resolution, 2 ward projection,
-- 3 roll-input accounting, 4a/4b/4c modifier composition, 5 brewer
-- selection) and the persisted Resolution Trace. Verbatim from
-- migration 0100.
--
-- Canonical source: this file is the source of truth for the function body.
-- Edit here and run `npm run build:migrations` -- do not hand-edit the
...
-- BEGIN db/sql/functions/start_round.sql
...
comment on function public.start_round(uuid) is
  ...
  'auto-declined first).';
-- END db/sql/functions/start_round.sql

-- BEGIN db/sql/functions/submit_roll.sql
...
-- END db/sql/functions/submit_roll.sql

-- BEGIN db/sql/functions/submit_roll_as.sql
...
-- END db/sql/functions/submit_roll_as.sql
