 ...
 * #208) — player header, Brew Rating card, Player Stats card, and a link
 * down to the collection route, per the picked "stacked stats-page mirror"
 * layout prototype variant (issue #203). Mirrors `SpellCollectionPage`'s
 * split between the route (auth/data-fetching) and this presentational
 * shell. `viewerPlayerId` seeds the backdrop's daily prop shuffle off the
 * signed-in viewer, same reasoning as `SpellCollectionPage`'s own prop —
 * the background doesn't change depending on whose profile is being viewed.
 */
