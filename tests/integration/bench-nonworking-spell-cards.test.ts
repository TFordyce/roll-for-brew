...
describe.skipIf(!hasAnonTestEnv)("spell cards: non-working cards benched from the draw pool", () => {
  ...
  it("benches only cards from the documented non-working list", async () => {
    ...
    // three round-scoped modifier snapshot cards (#343, migration 0087) and
    // the three durable persistent-modifier cards (#342, migration 0088) are
    // since implemented and un-benched, so 28.
    expect(new Set(BENCHED_SPELL_CARDS).size).toBe(28);
