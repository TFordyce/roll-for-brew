...
**Deferred Forced-Reroll Target**:
A pre-roll `forced_reroll` cast (Yorkshire Terror, or a WILD/TABLE fan-out) armed before its target was a round participant — `spell_casts.effect_kind = 'forced_reroll'` with `target_pending = true`, `target_player_id` null, no `reaction_window_id` — filled in by `set_spell_cast_target` after `close_round` (issue #325). Same layer-0 completeness hold as a Pending Spell Die: `get_current_layer_rolls_if_complete` / `get_completed_layer_rolls_for_stall_resolution` don't treat Layer 0 as complete while one is outstanding, because resolving now would silently drop the reroll — the cast never attaches to a reaction window, never reaches `apply_forced_reroll`, and so never records a `cast_inputs.roll_transform` for `resolve_round` Phase 3 to adopt. Once the caster names the target the app re-enters layer resolution (`afterDeferredCastTargetSet`), which opens the reaction window for the first time — `open_reaction_window`'s own attach pass then picks the no-longer-pending cast up, so the nobody-holds-a-Reaction path reaches the same end state as the reaction-window-stays-open path. Terminal outcome for a target the caster never names: the existing 5-minute closed-round stall timer force-negates the cast via `resolve_stalled_pending_forced_reroll_casts` (the same timer's own branch, not a new clock), leaving it a recorded no-op (`negated`, `cast_inputs.deferred_target_abandoned`) with no `roll_transform`, and the round resolves off the un-rerolled rolls.
...
**Resolver pipeline**:
The ordered internal phases `resolve_round` runs to produce a layer-0 outcome (ADR 0005, ADR 0006): **0a/0b** Effect Invocation, **1** Cast-Log resolution (negate / redirect / backfire), **2** ward projection, **3-pre** Calami-Tea per-round dice tick synthesis (#289), **3** roll-input accounting, **4a** modifier composition, **4c** lowest-gains-highest, **4b-pre / 4b** persistent modifier delta, **5** brewer selection. An internal authoring structure — the sub-phase headers carried in `resolve_round`'s body and its `comment on` — not a surface anything renders. Distinct from **Recap phase**, the player-facing display banding (`Before the roll` / `Reaction window` / `Outcome`): a resolver phase does not map one-to-one to a Recap phase, and the two must never be conflated.
